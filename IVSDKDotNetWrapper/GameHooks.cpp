...
void UnmanagedGameHooks::Initialize()
{
	...
	TryCreateHookAtCall("UnmanagedGameHooks", "RegisterNativeNoChecks", &RegisterNativeNoChecksHook, (void**)&originalRegisterNativeNoChecks);
	TryCreateHook("UnmanagedGameHooks", "IsNetworkGameRunning", &IsNetworkGameRunning, (void**)&originalIsNetworkGameRunning);
	TryCreateHookAtCall("UnmanagedGameHooks", "IsThisMachineTheServer", &IsThisMachineTheServer, (void**)&originalIsThisMachineTheServer);
...
bool UnmanagedGameHooks::TryCreateHookAtCall(const std::string& section, const std::string& key, LPVOID detour, LPVOID* ppOriginal)
{
	MH_STATUS mhStatus;

	LPVOID target = injector::GetBranchDestination(AddressSetter::Get(section, key)).get();
	mhStatus = MH_CreateHook(target, detour, ppOriginal);
